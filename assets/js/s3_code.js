...
(function(d, _scEnv){
	switch (d) {
		case "www.softbank.jp" :
		case "online-shop.mb.softbank.jp" :
		case "p4b.smktg.jp" :
			_scEnv.rsid = "sbmrobotprd";
			_scEnv.cdp = "3";
			_scEnv.env = "p";
			break;
		default :
			_scEnv.rsid = "sbmrobotdev";
			_scEnv.cdp = location.host.match(/\.(co|ne|or)\.jp$/)?"3":"2";
			_scEnv.env = "d";
			break;
	}
})(document.domain,_scEnv);

var s3_account = _scEnv.rsid;
var s3 = s_gi(s3_account);

/************************** CONFIG SECTION **************************/
/* You may add or alter any code config here. */
s3.visitorNamespace = "softbankmobile";
s3.trackingServer="softbankmobile.122.2o7.net";

s3.charSet="UTF-8";
s3.cookieDomainPeriods = _scEnv.cdp;
s3.linkInternalFilters = "javascript:,www.softbank.jp/robot,online-shop.mb.softbank.jp,p4b.smktg.jp";
if (_scEnv.env == "d") s3.linkInternalFilters += "," + location.hostname;
...
s3.linkTrackVars="server,prop72";
s3.linkTrackEvents="None";

/* Page Name Plugin Config */
s3.siteID=document.domain;
s3.defaultPage="index.html";
s3.queryVarsList="";	// query parameters to keep
s3.pathExcludeDelim=",";	// portion of the path to exclude
s3.pathConcatDelim="/";	// page name component separator
s3.pathExcludeList="";	// elements to exclude from the path
...
s3.doCmnStrPlugin = function() {

	var s = this;

	s.visitorID = s.manageVisitorID('fst_vi3');
	...
	// tracking code
	s.eVar17 = s.getQueryParam('utm_source');
	s.eVar18 = s.getQueryParam('utm_medium');
	s.eVar19 = s.getQueryParam('utm_term');
	s.eVar20 = s.getQueryParam('utm_content');
	s.eVar21 = s.getQueryParam('utm_campaign');

	s.eVar22 = s.getQueryParam('cid');
	s.eVar23 = s.getQueryParam('oid');
	s.eVar24 = s.getQueryParam('adid');


	s.prop17 = document.domain;
	s.eVar1 = "D=c17";

	sc_dirAll=sc_setDirName(location.pathname);
	s.prop18=sc_dirAll[0]?s.server+"/"+sc_dirAll[0]:s.server;
	s.prop3=sc_dirAll[1]?s.prop18+"/"+sc_dirAll[1]:s.prop18;
	s.prop4=sc_dirAll[2]?s.prop3+"/"+sc_dirAll[2]:s.prop3;
	s.eVar2="D=c18";
	s.eVar25="D=c3";
	s.eVar26="D=c4";

	if (document.title) s.prop6 = document.title.substring(0,50);
	s.eVar28="D=c6";

	s.prop7=document.URL;
	s.eVar29="D=c7";

	s.prop9 = s.getPercentPageViewed();
	s.eVar9 = "D=c9";

	s.prop10 = s.getPreviousValue(s.pageName,'sc_ppv_pagename3');
	s.eVar10 = "D=c10";

	var gvs=s.getVisitStart('s_gvs3');
	var vstInval=getVstInval(s,"s_fr3",s.visitTerm,gvs,"1");
	s.prop11=(vstInval==0)?"First visit":vstInval;

	vstInval=getVstInval(s,"s_lr3",s.visitTerm,gvs,"2");

	if (s.prop11 == "First visit") {
		s.prop12 = s.prop11;
	} else if (vstInval==0) {
		s.prop12 = "Same day";
	} else {
	...
	s.prop13=s.currentYear + "/" + s.currentM + "/" + s.currentD + " " + s.currentH + ":" + s.currentm + ":" + s.currentS;
	s.eVar13 = "D=c13";

	s.prop14=s.getNewRepeat(s.visitTerm,"s_nr3");
	s.eVar14 = "D=c14";

	s.prop15=s.eVar15="D=User-Agent";

	s.prop16=s.getTimeParting('p','9');
	s.eVar16 = "D=c16";
...
s3.doCmnLstPlugin = function() {
	var s = this;
	if(s.linkType) {
		s.linkTrackVars=s.apl(s.linkTrackVars,"server,prop72",",","1");
	}
};


s3.doPagePlugin = function() {
	...
	if (location.pathname == "/robot/d/form/customer/done/") {
...
function getVstInval(s,cNm,cTm,vstStrt,type) {
	...
	if (sc_arrVisitTime.length != 3 || isNaN(sc_visitTime.replace(new RegExp(":","g"),""))){ // mod 2014.12.04
		s.c_w(cNm,sc_presentTime,sc_fr_exp);
		if (type == "2") s.c_w(cNm+"tmp",sc_presentTime);
		return "0"
	}
 ...
 */
s3.trackFirstPartyCookie=new Function("c","e","l",""
 ...
 */
s3.getQueryParam=new Function("p","d","u",""
+"var s=this,v='',i,t;d=d?d:'';u=u?u:(s.pageURL?s.pageURL:s.wd.locati"
+"on);if(u=='f')u=s.gtfs().location;while(p){i=p.indexOf(',');i=i<0?p"
+".length:i;t=s.p_gpv(p.substring(0,i),u+'');if(t){t=t.indexOf('#')>-"
+"1?t.substring(0,t.indexOf('#')):t;}if(t)v+=v?d+t:t;p=p.substring(i="
+"=p.length?i:i+1)}return v");
s3.p_gpv=new Function("k","u",""
+"var s=this,v='',i=u.indexOf('?'),q;if(k&&i>-1){q=u.substring(i+1);v"
+"=s.pt(q,'&','p_gvf',k)}return v");
s3.p_gvf=new Function("t","k",""
 ...
 */
s3.getTimeParting=new Function("t","z",""
 ...
 * otherwise 0
 */
s3.getVisitStart=new Function("c",""
 ...
 * @returns vid
 */
s3.manageVisitorID = function(cn){
		...
		if(hr && hr.indexOf("#") ==-1 && hr_obj.href && hr.match(/^http[s]?:\/\/([a-zA-Z0-9\.\-\_]+)/i)){
			...
			if(document.domain != hr_domain){
				...
				for (var i = 0; i < lifAry.length ; i++) {
					if (hr_domain.indexOf(lifAry[i].replace(/\/.*/,"")) > -1) {
 ...
 */
s3.getNewRepeat=new Function("d","cn",""
 ...
 */
s3.getPercentPageViewed=new Function("",""
+"var s=this;if(!s.linkType||s.linkType=='e'){var v=s.c_r('s"
+"_ppv');s.c_w('s_ppv',0);return v;}");
s3.getPPVCalc=new Function("",""
+"var s=window.s3;var dh=Math.max(Math.max(s.d.body.scrollHeight,s.d.documentElement."
...
s3.getPPVSetup=new Function("",""
+"var s=this;if(s.wd.addEventListener){s.wd.addEventListener('load',s.getPPVCalc"
 ...
 */
s3.getPreviousValue=new Function("v","c","el",""
...
s3.getRandomStr=new Function("l",window.RegExp?"for(var o='c5iFO6ybR9wt"
